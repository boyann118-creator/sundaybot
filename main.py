...
def save_to_github():
    ...
    payload = {"message": "Update groups via Sunday Bot", "content": encoded_content, "sha": sha}
    resp = requests.put(url, headers=headers, json=payload)
    if resp.status_code in [200, 201]:
        print(f"[{time.strftime('%H:%M:%S')}] ✅ 远程 GitHub 仓库已更新")
        return True
    print(f"[{time.strftime('%H:%M:%S')}] ❌ GitHub 写入失败: {resp.text}")
    return False
