...
async def group_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb.append([InlineKeyboardButton("➕ 添加成員 (錄入 ID)", callback_data='add_m_start')])
...
async def add_mem_id_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text(f"👉 **目標分組：{context.user_data['active_g']}**\n\n請發送群組 **ID** (必須帶 -)：")
    return ADD_ID
...
async def add_mem_final_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ 已錄入群組：{remark}")
    return await start(update, context)
...
async def del_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer("成員已移除")
    return await group_detail(update, context)

async def del_group_full(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def bc_select_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    rows = db_op("SELECT name FROM group_names", is_select=True)
    if not rows:
        await update.callback_query.edit_message_text("❌ 暫無分組，請先創建。", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ 返回", callback_data='to_start')]]))
        return MAIN_STATE
...
async def bc_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ 發送完成！成功發送至 {count} 個群。\n\n**您可以繼續發送內容，或輸入 /start 返回。**")
    return BCAST_MSG
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            MAIN_STATE: [
                ...
                CallbackQueryHandler(bc_select_group, pattern='^start_bc$'),
            ],
    ...
    # 單獨掛載那些不屬於複雜對話流，但需要隨時響應的按鈕
    app_tg.add_handler(CallbackQueryHandler(add_mem_id_start, pattern='^add_m_start$'))
    app_tg.add_handler(CallbackQueryHandler(del_group_full, pattern='^del_g_all$'))
    app_tg.add_handler(CallbackQueryHandler(del_member, pattern='^rm_'))
